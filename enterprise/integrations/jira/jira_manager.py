...
class JiraManager(Manager):
    ...
    async def verify_webhook_signature(
        self, signature: str | None, body: bytes, payload: dict
    ) -> bool:
        ...
        if hmac.compare_digest(signature, digest):
            logger.info(
                '[Jira] Webhook signature verified successfully',
    ...
    async def receive_message(self, message: Message):
        ...
        is_job_request = await self.is_job_requested(message)
    ...
    async def is_job_requested(
        ...
        return JiraFactory.is_labeled_ticket(
            message
        ) or JiraFactory.is_ticket_comment(message)

    async def does_job_contain_details(
        ...
        try:
            ...
            if not provider_handler:
                logger.warning(
                    '[Jira] Job request failed: no provider tokens available',
                    extra={
            ...
            if result.success:
                ...
                jira_view.selected_repo = result.repository.full_name
                logger.info(f'[Jira] Verified repository: {result.repository.full_name}')
                return True
            else:
                # Log the specific failure reason
                logger.warning(
                    '[Jira] Job request failed: repository verification unsuccessful',
                    extra={
            ...
            logger.error(
                f'[Jira] Error in is_job_requested: {str(e)}',
